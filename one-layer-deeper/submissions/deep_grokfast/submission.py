"""Multi-layer deep Transformer with Grokfast — no recurrence.

Key insight shift: maybe the recurrent weight-tying is HURTING us.
Grokking research used simple MLPs and 1-2 layer transformers.
Let's try a deeper non-recurrent transformer with Grokfast + high WD.

The baseline is 1 block. We go deeper (4 blocks, no sharing) to give
the model more computational steps without the constraint of weight tying.
Each block can specialize for a different phase of the computation.
"""
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor | None) -> Tensor:
        ...
        # ReGLU-style FFN (better for arithmetic than GELU)
        h = self.up(self.nf(x))
