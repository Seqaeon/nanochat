...
Closing this throughput gap is the primary practical challenge for deployment. The most promising directions are restructuring the computation to use fewer, larger cuBLAS calls; distilling to $K{=}1$ templates at inference time (recovering dense-equivalent throughput); or leveraging cuBLAS-based batched GEMM formulations that avoid custom kernels entirely.
...
\begin{figure*}[t]
    ...
    \caption{CORE score vs.\ model size and compute. \textbf{Left:} Total parameters --- Remix falls on the dense curve, reflecting $K{\times}$ storage. \textbf{Middle/Right:} Active parameters and active FLOPs --- Remix achieves 34--36\% higher CORE scores at matched compute, confirming the scaling improvement is not a parameter-counting artifact.}
    \label{fig:core_scaling}
\end{figure*}

Figure~\ref{fig:core_scaling} plots CORE score against active FLOPs. Remix d8 achieves a CORE score 36\% above the dense power-law prediction (0.123 vs.\ 0.090), and Remix d12 achieves 35\% above (0.172 vs.\ 0.127), reaching a CORE level between the dense d17 and d20 baselines. These gains confirm that BPP improvements translate to genuine downstream task performance.
