...
parameters; MST reaches dense wall-clock parity at $\dmodel{=}1536$ where MoL
reports its own sparse path losing to dense; and MoL's central mechanism, gating
...
\paragraph{3. Wall clock.} MST reaches training parity with the dense baseline at
$\dmodel{=}1536$ (76.1\,ms against 77.2\,ms per step) and is $1.30\times$ faster
in prefill at $T{=}32768$. MoL's own Section~5.6 reports the opposite for its
sparse path: $2.8\times$ slower than its batched fallback on two hardware classes,
decode slower than dense in absolute terms at every measured context, and dense
still ahead on prefill at $T\!\leq\!2$K. Both architectures reduce FLOPs; only one
of them currently converts that into time.
