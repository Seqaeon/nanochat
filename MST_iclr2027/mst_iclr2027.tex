...
compute-optimal point so that it is not disadvantaged.
...
anchoring the budget on MoL's own compute-optimal point (3.658B tokens) so that MoL
cannot be called undertrained. Both arms now carry their per-block or per-stream
value-embedding variant, matching the configuration each paper actually proposes.
...
MoL is at its compute-optimal point here; MST is $3.13\times$ past its own, which
inflates its per-token multiplier and is stated below.}
...
\midrule
MST $\nsub{=}4$        & 0.8281 & $\mathbf{1.997\times}$ & $0.938\times$
                       & $\mathbf{2.633\times}$ & $\mathbf{1.780\times}$ & $\mathbf{0.982\times}$ \\
MoL $1{+}3\text{of}15$ & \textbf{0.8062} & $1.849\times$ & $\mathbf{1.143\times}$
...
it leads on training FLOPs ($1.143\times$ against $0.938\times$), consistent with
...
$3.13\times$ past its own compute-optimal point, and overtraining improves \bpb\ at
fixed FLOPs per token. The $1.997\times$ is therefore not comparable to the
