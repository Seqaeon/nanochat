...
Standard MoE requires auxiliary load-balancing losses to prevent expert collapse \citep{fedus2022switch}. We replace this with quantile-balanced routing. Concretely, we maintain a per-expert EMA threshold $\boldsymbol{\theta}_k$ updated each step as $\boldsymbol{\theta}_k \leftarrow \lambda\,\boldsymbol{\theta}_k + (1{-}\lambda)\,q_k$, where $q_k$ is the $(1 - \text{topk}/K)$-quantile of expert $k$'s batch scores and $\lambda{=}0.99$. A token is assigned to expert $k$ if its score exceeds $\boldsymbol{\theta}_k$, unioned with a hard top-$k$ fallback to guarantee at least $k$ active experts per token; excess selections are trimmed to the highest-scoring $k$. The resulting mask is applied before a masked softmax, yielding differentiable routing weights. This ensures balanced template utilization without introducing an additional loss term or its associated hyperparameter.
...
\paragraph{Standard MoE fails at this scale.} Both MoE variants underperform the dense baseline, indicating that coarse expert routing is insufficient when the individual expert capacity is limited. We note that the MoE baselines were trained with a token budget computed from \emph{total} parameters (matching the dense baseline's Chinchilla ratio); preliminary results with active-parameter-matched budgets showed similar trends but are not included here.

\subsection{Inference Throughput}
\label{sec:throughput}

While RemixedLinear improves quality at matched \emph{active} FLOPs, its wall-clock inference throughput is lower than the dense baseline due to higher total parameter count and factored computation. Table~\ref{tab:throughput} reports measurements on a single NVIDIA H200 GPU at batch size 64, sequence length 2048, without \texttt{torch.compile}.

\begin{table}[t]
\centering
\small
\caption{Inference throughput at d12 on a single H200 GPU (batch 64, seq 2048). Hardware FLOPs are measured via \texttt{torch.utils.flop\_counter}. RemixedLinear achieves 10\% better BPP but at 3.7$\times$ lower throughput, primarily due to memory bandwidth constraints from $K{\times}$ total parameters.}
...
The hardware FLOP ratio is only 1.6$\times$ (3.6e8 vs.\ 2.2e8), yet throughput differs by 3.7$\times$. This gap is dominated by \textbf{memory bandwidth}: RemixedLinear loads 2.8$\times$ more parameters from HBM per forward pass (792M vs.\ 286M), and the many small per-chunk operations prevent efficient GPU utilization (86 vs.\ 195 TFLOPS). The architecture is memory-bandwidth bound rather than compute bound at this batch size.

This throughput gap is analogous to standard MoE: Mixtral-8$\times$7B has 47B total parameters but 13B active, yet is substantially slower than a 13B dense model in practice. The gap is dominated by HBM memory bandwidth rather than compute (1.6$\times$ more hardware FLOPs but 3.7$\times$ lower throughput). Kernel fusion of the template assembly step does not address the root cause; the most promising directions are INT8 quantization of the template bank (halving memory bandwidth) and weight distillation to reduce $K$ at inference time.
...
The 22 tasks span five categories:
\begin{itemize}
    \item \textbf{Commonsense reasoning:} HellaSwag (0-shot and 10-shot), PIQA, WinoGrande, Winograd, COPA, CommonsenseQA, OpenBookQA
    \item \textbf{World knowledge:} ARC-Easy, ARC-Challenge, Jeopardy, BigBench QA WikiData
    \item \textbf{Reading comprehension:} BoolQ, SQuAD, CoQA, LAMBADA
    \item \textbf{Algorithmic reasoning:} BigBench Dyck Languages, BigBench CS Algorithms, BigBench Operators, BigBench Repeat Copy Logic
    \item \textbf{Other:} AGI Eval LSAT-AR (logical reasoning), BigBench Language Identification
\end{itemize}

Table~\ref{tab:core} reports the aggregate CORE score for each model configuration.

\begin{table}[t]
\centering
\small
\caption{CORE benchmark scores (22-task centered accuracy aggregate). Higher is better. RemixedLinear consistently outperforms the dense baseline on downstream tasks, confirming that BPP improvements transfer to in-context learning performance.}
\label{tab:core}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{Active Params} & \textbf{Val BPP} & \textbf{CORE} \\
\midrule
Dense d4 & 37M & 1.170 & \textit{TBD} \\
Dense d8 & 126M & 0.969 & \textit{TBD} \\
Dense d12 & 286M & 0.903 & \textit{TBD} \\
\midrule
Remix d4 & 37M & 1.082 & \textit{TBD} \\
Remix d8 & 127M & 0.903 & \textit{TBD} \\
Remix d12 & 290M & 0.814 & \textit{TBD} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[t]
    ...
    \caption{CORE benchmark score vs.\ active FLOPs. RemixedLinear (pink circles) achieves higher CORE scores than the dense scaling curve (blue squares, dashed fit) at matched active compute, confirming that the BPP scaling advantage translates to downstream task performance.}
    \label{fig:core_scaling}
\end{figure*}

Figure~\ref{fig:core_scaling} plots CORE score against active FLOPs, analogous to the BPP scaling analysis in Figure~\ref{fig:scaling}. The RemixedLinear points fall consistently above the dense scaling curve, confirming that the BPP improvements documented in Section~\ref{sec:experiments} translate to genuine downstream task gains rather than reflecting only compression-metric artifacts.
