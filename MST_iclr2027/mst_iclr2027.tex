...
$T{\approx}190$K and is $1.10\times$ faster at $T{=}262144$, because its windowed
streams scale as $T^{1.41}$ against dense's $T^{1.70}$, where MoL reports its own
sparse path losing to dense; and MoL's central mechanism, gating
...
as $T^{1.41}$ against dense's $T^{1.70}$, so matched \emph{on quality} MST
overtakes dense beyond $T{\approx}190$K and is $1.10\times$ faster at
$T{=}262144$ (Table~\ref{tab:longctx}). MoL reports the opposite for its own
...
The consequence is a crossover rather than a constant gap
(Table~\ref{tab:longctx}). Matched on quality, MST at $L{=}24$ is $0.72\times$
dense's prefill speed at $T{=}65536$, $0.90\times$ at $T{=}131072$ and
$\mathbf{1.10\times}$ at $T{=}262144$; the curves cross near $T{\approx}190$K,
bracketed by measurements on either side. Matched on FLOPs per token the
crossover is later, near $T{\approx}336$K; matched on parameters MST is ahead at
every length measured. This is the one wall-clock axis on which we claim an
advantage, and it is not an artifact of comparing models at equal depth: the
dense arm here is the one that reaches MST's \bpb.
...
the two measured neighbours. Ratios above $1$ mean MST is faster.}
\label{tab:longctx}
\begin{center}
\begin{small}
