...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --p26-output-gated-linear|--remix-basis-gate-rank|--remix-basis-gate-mode)
            ...
            shift 2
            ;;
        --p28-shared-basis|--p28-chunk-routing-size|--p28-global-template-bank|--p28-attn-proj-templates|--p28-attn-qk-templates|--target-active-params)
            EXTRA_ARGS+=("$1" "$2")
            shift 2
            ;;
        ...
        --eet-use-override|--eet-override-prob-start|--eet-override-prob-end|--eet-reenter-final|--eet-compute-skip|--eet-target-active-frac|--eet-capacity-schedule|--eet-exit-fracs|--eet-capacity-alignment-lambda|--eet-router-task-grad|--eet-reinforce-interval|--eet-reinforce-lambda)
            EXTRA_ARGS+=("$1" "$2")
            shift 2
            ;;
        *)
