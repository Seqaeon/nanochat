...
\begin{figure}[t]
\begin{center}
\fbox{\parbox[c][2.4cm][c]{0.95\textwidth}{\centering \TODO{Fig.~4: (a)
$\Delta$\bpb\ from zeroing each stream: $+5.19, +7.77, +4.44, +6.09$;
(b) per-layer cosine similarity, $0$ to $+0.64$, with the $-0.333$ floor marked;
(c) router entropy vs layer against $\log\nsub=1.386$.}}}
\end{center}
\caption{Every stream carries load and the streams stay approximately
orthogonal. Damage from deleting a stream varies by only $1.75\times$, against
the $7.2\times$ gradient imbalance of an untreated partition.}
\label{fig:subablate}
\end{figure}
